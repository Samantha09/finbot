...
3. `getPositionReport(days=7)` — 获取最近 7 天（或用户指定的 30 天）持仓历史趋势汇总，作为调仓建议的参考背景
4. `etfRotationStrategy(period, mode, holdings)` — 基于当前持仓和历史趋势给出轮动调仓建议
   - `period`：默认 `"medium"`，若用户指定周期则按用户要求
   - `mode`：使用 `"auto"`（自动从全市场按主题选基）
   - `holdings`：从用户持仓中提取，格式为 `[{"symbol":"510300.SH","ratio":0.3}]`，`ratio` 按 `市值/总资产` 计算
   - 调用前必须将步骤 3 的历史回顾数据作为上下文融入建议中
